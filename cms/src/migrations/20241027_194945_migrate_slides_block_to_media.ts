...
export async function up({ payload }: MigrateUpArgs): Promise<void> {
  ...
  for (const page of pages) {
    ...
    for (const block of page.hero as any[]) {
      if (block.blockType !== "Slides") continue;

      for (const slide of block.slides) {
        const altByLocale = slide.image["alt"] as Record<string, string>;

        slide["imageAlignment"] = slide.image["alignment"];
        slide["image"] = await migrateMedia(slide["image"].url, altByLocale);
      }
    ...
    if (updatePage) {
      await payload.db.connection
        .collection("pages")
        .updateOne({ _id: page._id }, { $set: { hero: page.hero } });
    }
  }
