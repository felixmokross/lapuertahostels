...
export async function up({ payload }: MigrateUpArgs): Promise<void> {
  ...
  for (const page of pages) {
    ...
    for (const block of page.layout as any[]) {
      if (block.blockType !== "AccommodationSelector") continue;

      for (const card of block.cards) {
        const altByLocale = card.image["alt"] as Record<string, string>;

        card["image"] = await migrateMedia(card["image"].url, altByLocale);
      }
