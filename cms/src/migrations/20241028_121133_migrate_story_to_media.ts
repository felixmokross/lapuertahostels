...
export async function up({ payload }: MigrateUpArgs): Promise<void> {
  ...
  for (const page of pages) {
    ...
    for (const block of page.layout as any[]) {
      if (block.blockType !== "Story") continue;

      if (block.image && (block.image.show || block.image.show == null)) {
        if (block["image"].position) {
          block["imagePosition"] = block["image"].position;
        }

        if (block["image"].grayscale) {
          block["grayscaleImage"] = block["image"].grayscale;
        }
